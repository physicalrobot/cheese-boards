...
describe('User and Board,Cheese Models', () => {
    ...
    test('User assocation to board', async () => {
        const cheese = await Cheese.create({ title: "Pepper Jack", description: "spicy and good" });
        expect(cheese.title).toBe("Pepper Jack");
        ...
        const cheeseboard3 = await Board.create({ type: "wood", description: "medium", rating: 3 })

        await nishu.addBoards(cheeseboard2);
        await nishu.addBoards(cheeseboard3);

        const theboards = await nishu.getBoards()

        expect(theboards.length).toBe(2)
